...
NAME = "HalfCheetaModified"

def runHalfCheetaModified(steps):
    seed = np.random.randint(0, 2**31 - 1)

    env = make_vec_env('HalfCheetaModified', n_envs=24, seed=seed)

    seed = np.random.randint(0, 2**31 - 1)
    vec_env = make_vec_env('HalfCheetaModified', n_envs=4, seed=seed)
