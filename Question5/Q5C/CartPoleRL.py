...
def runCartPole(steps):
    seed = np.random.randint(0, 2**31 - 1)
    env = make_vec_env('CartPole-v1', n_envs=24, seed=seed)

    seed = np.random.randint(0, 2**31 - 1)
    vec_eval = make_vec_env('CartPole-v1', n_envs=4, seed=seed)
    ...
    env_eval = VecVideoRecorder(vec_eval, f'./Question5/Q5C/data/videos', record_video_trigger=lambda x: x == 0, video_length=500, name_prefix=f"{NAME}")
