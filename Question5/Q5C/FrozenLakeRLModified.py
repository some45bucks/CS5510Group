...
NAME = "FrozenLakeModified"

def runFrozenLakeModified(steps):
    seed = np.random.randint(0, 2**31 - 1)
    env = make_vec_env('FrozenLakeModified', n_envs=24, seed=seed)

    seed = np.random.randint(0, 2**31 - 1)
    vec_eval = make_vec_env('FrozenLakeModified', n_envs=4, seed=seed)
